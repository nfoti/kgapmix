% Run finite smapler on CMB data with box kernel.
%
...
%for si = 2:numel(seeds)
  ...
  Kern = sekern(XXtr, mus, mu_inds, psis, psi_inds);
  ...
  samplePhi = true;
  singlePhi = true;
  phi = 1/0.02; % Determined from the data
  ...
  fprintf('Burn in...\n');
  params = kgap_gmm_finite(TTtr,XXtr,init_burn, '_cmb_se_finite3');
  ...
  fprintf('Sampling...\n');
  params = kgap_gmm_finite(TTtr,XXtr,init,'_cmb_se_finite3');
  ...
  system(['mkdir -p cmb_se_finite']);
  save(['cmb_se_finite/cmb_se_finite_' num2str(si) '.mat'], 'pll_finite', 'pl_finite', 'pl2_finite', ...
