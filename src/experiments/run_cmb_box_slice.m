% Run slice sampler on CMB data with box kernel.
%
...
for si= 2:numel(seeds)
  ...
  mus = unique(MMtr);
  psis = .1; % kern_tau in sngp version
  ...
  system('mkdir -p cmb_box_slice');
  save(['cmb_box_slice/cmb_box_slice_' num2str(si) '.mat'], 'pll_slice', 'pll_slice_rb', 'pl_slice', 'plse_slice', ...
