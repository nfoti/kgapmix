function h = gen_pred_waterfall_finite(fn,Y,X,times,xstr,ystr)
...
h = zeros(1,ntime+1);
nsamp = numel(KK); % The only variable with the number of samples
...
%                 pl_slice(XXind==i)', 'LineWidth',2);
  tt = [times(i).*ones(size(predvals)); predvals; pl_lsp_finite(XXind==i)/nsamp];
